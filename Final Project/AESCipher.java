 ...
 * the methods to perform the operations of AES Algorithm
 */
...
public class AESCipher {
  ...
  public HashMap<Integer, String> aesRoundKey(String keyHex) {
    ...
    //storing the input key into ke[4*6] or ke[4*8] matrix
    ...
    for (int j = columns; j < (W[0].length); j++) {
      ...
      if (j % columns == 0) {
        ...
        if (j % 4 == 0 && W[0].length == 60) {
          ...
          for (int r = 0; r < 4; r++) {   // Perform the S-box substitution plus the rcon XOR
            y[r] = aesSBox(y[r]);
          }
          w = xor(x, y);
          for (int i = 0; i < 4; i++) {
  ...
  public  void encryption(String message, int encryptType) {
...
//         addKey values will be stored in the outStateXor for every roundkey
    ...
    System.out.println(encryptType + "-bit encrypted message is " + cipherText);
  }
...
//    System.out.println(decryptType + "-bit decrypted message is " + cipherText);
    return cipherText;
  }
}
